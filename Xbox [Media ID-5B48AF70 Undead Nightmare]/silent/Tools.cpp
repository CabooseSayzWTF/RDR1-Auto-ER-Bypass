	...
	FARPROC ResolveFunction(CHAR* ModuleName, DWORD Ordinal) 
{
	...
	VOID XNotifyDoQueueUI2(PWCHAR pwszStringParam) {
		toggleNotify(true);
		XNotifyQueueUI(XNOTIFYUI_TYPE_AVOID_REVIEW, XUSER_INDEX_ANY, XNOTIFYUI_PRIORITY_HIGH, pwszStringParam, NULL);
		toggleNotify(false);
	}
	...
	VOID NotifyPopup2(PWCHAR myPWCHAR) {
		if (KeGetCurrentProcessType() != PROC_USER) {
			HANDLE th = CreateThread(NULL, NULL, (LPTHREAD_START_ROUTINE)XNotifyDoQueueUI2, (LPVOID)myPWCHAR, CREATE_SUSPENDED, NULL);
			if (th == NULL) return; ResumeThread(th);
		}
		else XNotifyDoQueueUI2(myPWCHAR);
	}